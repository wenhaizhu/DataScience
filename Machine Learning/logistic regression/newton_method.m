function [cross_entropy, w, b] = newton_method(step, data, w, b)
...
for s=1:step
    ...
    inner = pinv(trans_x * delta * transpose(1 - delta) * features) * trans_x * (delta - class);
    w = w - transpose(inner);
    
    %inner_b2 = transpose(delta) * (1 - delta);
